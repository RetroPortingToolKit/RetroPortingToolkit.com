...
export function itemCoverSource(item) {
  const cover = typeof item.cover === "string" ? item.cover : "";
  let file = null;
  if (cover.startsWith("./") && item.dir) {
    file = path.join(item.dir, cover.slice(2));
  } else if (cover.startsWith("/data/")) {
    file = path.join(ROOT, cover.slice(1));
  }
  if (!file) return null;
  // A frontmatter path is data, not permission to read arbitrary files. Keep
  // relative covers beside their item and absolute covers under data/ before
  // checking existence.
  const allowedRoot = cover.startsWith("/data/")
    ? path.resolve(DATA_DIR) + path.sep
    : path.resolve(item.dir) + path.sep;
  const resolved = path.resolve(file);
  if (!resolved.startsWith(allowedRoot)) return null;
  if (!fs.existsSync(resolved)) return null;
  // Prefer the same-stem WebP generated by scripts/gen-image-variants.mjs;
  // retaining the original beside it preserves provenance and easy rollback.
  const ext = path.extname(resolved).toLowerCase();
  const webp = ext === ".png" || ext === ".jpg" || ext === ".jpeg"
    ? resolved.slice(0, -ext.length) + ".webp"
    : null;
  return webp && fs.existsSync(webp) ? webp : resolved;
}

export function itemCoverPublicPath(item) {
...
export function itemImagePath(item) {
...
export function itemCoverAlreadyServed(item) {
  const cover = typeof item.cover === "string" ? item.cover : "";
  if (/^https?:\/\//.test(cover)) return cover;
  if (cover.startsWith("/") && !cover.startsWith("/data/")) {
...
export function absoluteImage(img, origin) {
