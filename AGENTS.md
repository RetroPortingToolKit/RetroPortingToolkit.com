...
- **Pushing main deploys production through the existing Vercel connection.** For code/config/content changes, run `npm run typecheck`, `npm run build`, and `npm run test` first; prose-only developer docs need reference/structure checks. Publish only ready material; use drafts instead of holding dirty/unpushed work. Other deployments require owner intent; do not add automatic jobs/hooks/CI.
