...
export class SubmissionStore {
  ...
  async create(snapshot: Snapshot, record: Submission) {
...
export async function repositoryMetadata(repo: string) {
  ...
  return { repo: canonical, title: plainText(data.name, 100), description: plainText(data.description, 500), owner: plainText(url.hostname === 'github.com' ? data.owner?.login : data.namespace?.full_path, 150) || project.split('/').slice(0, -1).join('/') };
}
export async function submitRepository(store: SubmissionStore, input: Record<string, unknown>, now = new Date()) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try { await store.create(snapshot, record); return { record, duplicate: false }; }
