...
Per its release notes, the port runs bare metal on original Xbox hardware with full audio, cheats, and ten save-state slots managed through the Xbox dashboard's save system, persisting across sessions with no manual setup. Controls map to the Xbox pad: D-pad to move, A to jump, X to shoot with hold-to-charge, B to dash, triggers and White/Black to switch weapons.

## Technical details

The port sits on mstan's [MegaManXSNESRecomp](/games/mega-man-x), which statically recompiles the SNES 65816 CPU code to C while the rest of the console (PPU rendering, APU audio, DMA, hardware registers) runs through the [snesrecomp](/hardware/super-nintendo) runtime. Team Resurgent's contribution is a custom XDK platform layer in Platform/xbox: Direct3D presentation, Xbox audio and input backends, and savegame plumbing through the dashboard's UDATA storage, replacing the SDL layer the PC builds use.
