...
A toolchain in this fleet is two programs: a recompiler that turns a game's machine code into C at build time, and a runtime the generated C links against. Building one gets you the tools, not a playable game, and that is the right place to stop and check your work: every framework here has a way to prove it built correctly with no game file present. This guide is the clone-to-binary path for each framework, using only the commands the projects themselves publish.

## What each toolchain requires

Read this as what the repository claims, not as a portability promise. Several projects state plainly that a platform is untested, and those words are kept here.
...
Three gaps are worth naming rather than filling. ndsrecomp does not state a supported operating system list. gbrecompiled states requirements for its Android target but not for desktop hosts. vbrecomp names no compiler version or standard. If you need certainty on any of those, the repository is the place to ask, not this page.

The table covers this fleet's console toolchains. Two neighbours have their own instructions and are not repeated here: the upstream projects, which validate in CI on Ubuntu, Windows and macOS and want CMake 3.20 or above with a C++20 compiler, and the Xbox probe, which builds through nxdk pinned to the commit in its `.nxdk-version` rather than through CMake at all.

## Two rules that apply to every build here

**The build type is not optional.** Generated C is enormous, and psxrecomp is explicit about what happens if you configure without one, in [`docs/BUILDING.md`](https://github.com/mstan/psxrecomp/blob/master/docs/BUILDING.md):
...
The same choice decides whether you get a debug surface at all. On psxrecomp, `PSX_DEBUG_TOOLS` defaults on for Debug and RelWithDebInfo and off for Release, so a Release build has no TCP debug server. [`gcnlle/build.sh`](https://github.com/mstan/gcnlle/blob/master/build.sh) encodes the same lesson as a comment on its own default, RelWithDebInfo: never configure without one.

**Parallelism is a memory constraint, not a speed dial.** Two repositories recorded this independently. From [`RELEASE.md`](https://github.com/mstan/SuperMarioWorldRecomp/blob/main/RELEASE.md) in SuperMarioWorldRecomp:
...
psxrecomp says the same about `config_loader.cpp` and gives the remedy: if `cmake --build` dies with no diagnostic, retry with `-j 2` or `-j 1`.

## Build the PlayStation toolchain

psxrecomp has the fullest documented build in the fleet, and it is the one to read first even if your console is elsewhere, because its three-stage shape is the shape the others repeat. A plain clone is enough for the recompiler and the runtime. Add `--recurse-submodules` only if you intend to build the launcher (`recomp-ui`) or netplay (`lib/recomp-net`); configuring with `-DPSX_RECOMP_UI=ON` without the submodule present is a `FATAL_ERROR`.
...
On macOS `brew install ninja cmake`, on Debian and Ubuntu `sudo apt install build-essential cmake ninja-build`. Then the three stages, verbatim from [`docs/BUILDING.md`](https://github.com/mstan/psxrecomp/blob/master/docs/BUILDING.md):
...
**You should now see** `build/recompiler/Release/NESRecomp.exe` on Windows, or `build/recompiler/NESRecomp` with no suffix elsewhere. Running it with a ROM path you supply is the next step, covered in [port a game](/docs/guides/port-a-game).

### snesrecomp

The recompiler is a Rust crate and the shipped CLI is packaged with PyInstaller:
...
**You should now see** a printed backend line when a consuming game repository configures: the SDL3 build reports the SDL3 desktop backend for both the game target and recomp-ui, and the SDL2 build reports the SDL2 compatibility backend instead. Keep the two backends in separate build directories so stale CMake package paths cannot leak between them.
...
On Windows the redistributable CLI is built separately with `py -3.12 tools/build_cli.py`, and the archive is written under `build/cli-release/`. Build with MSYS2 MinGW on Windows, not MSVC: `bios_hle.cpp` uses `__builtin_clz`, and MSVC fails with `error C3861`.
...
**You should now see** a green CTest run. One test is deliberately absent from it: `l1_decoder_test` is built but not registered with CTest because it requires a user-supplied Sonic ROM.

### vbrecomp

The Virtual Boy runtime can be built and proven with no cart at all, which makes it the cheapest verification in the fleet:
...
**You should now see** a runtime linked against `no_game_linked.c`: it starts, the TCP debug server answers, and no cart code is present. That is the documented expected result, not a failure.

### gcnlle

One helper script builds everything, and three CTest suites verify it:
...
The repository is candid that the middle of this is not turnkey: the full runner additionally expects firmware RAM-bank captures matching the configs under `bios/firmware_banks/`, and it says this bootstrap workflow still assumes an active developer setup.
...
**You should now see** `build/bin/gbrecomp`, which then generates a project from a ROM you supply and builds it with a second CMake invocation.

## Which of these instructions are the projects' own

All of them. Every sequence above is copied from the repository's README or its `docs/BUILDING.md`, and where a project does not document a step this page says so rather than filling it in: ndsrecomp's firmware capture, gbrecompiled's desktop prerequisites, vbrecomp's compiler requirement.

One thing on this page was reconstructed rather than read from a documented procedure, and it is worth flagging because it shapes how the whole fleet builds. The three-stage pattern named here, recompiler then regenerate then runner, was drawn out of a real build script, [`build_all.bat`](https://github.com/mstan/FaxanaduRecomp/blob/master/build_all.bat) in FaxanaduRecomp, rather than from prose any project wrote. The stage numbering and the distinct exit code per stage are that file's own. Treat the pattern as a description of what the scripts do, not as a rule any framework published.
...
- Upstream, for comparison: [`N64Recomp/README.md`](https://github.com/N64Recomp/N64Recomp/blob/main/README.md) and [`.github/workflows/validate.yml`](https://github.com/N64Recomp/N64Recomp/blob/main/.github/workflows/validate.yml)
...
- [What you need](/docs/start/what-you-need) is the shorter prerequisites answer, plus the game file contract.
- [Port a game](/docs/guides/port-a-game) is the step after this one, turning a built toolchain on a game file you supply.
- [Command line reference](/docs/reference/cli) has every flag on every tool built here.
- [Checking your own work](/docs/agents/verification-rituals) is the per-repository table of build and test commands.
