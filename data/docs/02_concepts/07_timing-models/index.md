...
Recompiled code runs as fast as the host can go, which is exactly the problem: the original console did not. Every project therefore has to decide how finely it models the passage of guest time, and that one decision sets both the speed of the port and the class of bugs it will have. The fleet spans the whole axis, from a whole frame executed as a few sequenced calls at one end to a model that advances hardware at the exact sub-instruction cycle at the other. None of them is the right answer generally. What selects between them is what the game's own code does with time.

## The axis

| Model | Granularity | Cost | Where it is used |
|---|---|---|---|
| Whole frame as sequenced calls | frame | fastest, and the compiled path has no per-instruction hook at all | snesrecomp shipping model |
| Per-instruction T-states, device advanced per scanline | scanline | one tick call per function body | smsggrecomp function form |
| Per-instruction cycle charge, lazy device catch-up | event-scheduled | one add per instruction plus a flush | gbarecomp |
| Tick before the access, read-modify-write split | per M-cycle for the marked accesses | split ticks per access | gbrecompiled |
| Flat single-step | one instruction per host call | highest, the host regains control every instruction | smsggrecomp `--flat-step` |
...
## The frame model: a whole frame as a few calls

At the other end, [`snesrecomp`](https://github.com/mstan/snesrecomp) runs each frame as discrete sequenced host calls.
...
## Cycle exact: advancing hardware at the right cycle

[`gbrecompiled`](https://github.com/mstan/gbrecompiled) is the project pushing the other way, and it has the most precise statement of what a whole-instruction model gets wrong. From [`MEM_TIMING_SCOPING.md`](https://github.com/mstan/gbrecompiled/blob/master/MEM_TIMING_SCOPING.md):
...
Start from what the guest does with time. A processor that has to interleave with other hardware every instruction wants flat stepping, whatever it costs. A game that reads a hardware counter mid-instruction, reprograms waitstates, or drives an effect from a raster interrupt will expose a frame or whole-instruction model, and the burndown row that records the result is axis 2, cycle and timing. A game that does none of those things runs correctly under a much cheaper model, and the cheaper model is then the right engineering choice.
...
- gbrecompiled: [`MEM_TIMING_SCOPING.md`](https://github.com/mstan/gbrecompiled/blob/master/MEM_TIMING_SCOPING.md), [`CYCLE_EXACT_INITIATIVE.md`](https://github.com/mstan/gbrecompiled/blob/master/CYCLE_EXACT_INITIATIVE.md), [`runtime/src/interpreter.c`](https://github.com/mstan/gbrecompiled/blob/master/runtime/src/interpreter.c), [`runtime/include/gb_timing.h`](https://github.com/mstan/gbrecompiled/blob/master/runtime/include/gb_timing.h)
