...
A recomp gives you the game running outside an emulator, on a runtime you own: renderer, audio, input, saves, frame loop. If that runtime serves many games, it also hands you everything the other games needed. Furthermore, it also means you can typically carry forward some of the supplemental *features* that were implemented for those games. In the case of [psxrecomp](https://github.com/mstan/psxrecomp/tree/master), a Playstation recompilation can now ship out of the box with [save states, rewind](https://www.youtube.com/watch?v=L36ppNkuJG0), and even netplay: features rarely seen in decomps / recomps historically, and often times believed outright not achievable within the ecosystem.
